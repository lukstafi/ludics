...
**Salvage on rejection**: If the reviewer rejects a declared scope expansion, first verify the rejection isn't a stale-base false positive, then capture the diff before reverting so nothing useful is lost:

0. **Verify the rejection isn't a stale-base false positive** — first resolve the base branch and the fork point (`BASE=$(git symbolic-ref --quiet --short refs/remotes/origin/HEAD 2>/dev/null | sed 's@^origin/@@' || echo main); MERGE_BASE=$(git merge-base "origin/$BASE" HEAD 2>/dev/null)`). If `[ -z "$MERGE_BASE" ]` (orphan branch, unfetched remote, shallow clone), neither arm below is conclusive — fall through to ordinary salvage rather than push back, since "no shared ancestor" means we cannot distinguish drift from genuine addition. Otherwise, for each rejected path two stale-base shapes mean "push back, do not revert": (a) **shared history at fork** — `[ -n "$MERGE_BASE" ] && git cat-file -e "$MERGE_BASE:<path>"` succeeds, meaning the file existed at the fork point and is not a scope expansion you added; (b) **fork-point-vs-tip drift** — `[ -n "$MERGE_BASE" ] && ! git cat-file -e "$MERGE_BASE:<path>"` (file absent at fork point) AND `git cat-file -e "origin/$BASE:<path>"` succeeds AND `git cat-file -e HEAD:<path>` fails, meaning `main` added the file *after* the fork and it was never on the branch. In either case, push back with the per-commit diff (`git log "$BASE..HEAD" --stat`, then `git diff <commit>^..<commit> --stat` for the suspect commit) instead of reverting shared history; skip the salvage flow below. The merge-base form supersedes the legacy `BASE:<path>` + `HEAD:<path>` conjunction, which resolved `$BASE` to the current `origin/$BASE` tip and missed arm (b).
