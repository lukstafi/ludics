...
0. **Verify the rejection isn't a stale-base false positive** — first resolve the base branch (`BASE=$(git symbolic-ref --quiet --short refs/remotes/origin/HEAD 2>/dev/null | sed 's@^origin/@@' || echo main)`), then for each rejected path run `git cat-file -e "$BASE:<path>"` and `git cat-file -e HEAD:<path>`. If both succeed, the file is shared history with the base branch, not a scope expansion you added — the reviewer is reading drift from a stale base. Push back with the per-commit diff (`git log "$BASE..HEAD" --stat`, then `git diff <commit>^..<commit> --stat` for the suspect commit) instead of reverting shared history. Skip the salvage flow below in that case.
