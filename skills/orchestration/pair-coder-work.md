...
0. **Verify the rejection isn't a stale-base false positive** — first resolve the base branch (`BASE=$(git symbolic-ref --quiet --short refs/remotes/origin/HEAD 2>/dev/null | sed 's@^origin/@@' || echo main)`) and the fork point (`MERGE_BASE=$(git merge-base "origin/$BASE" HEAD 2>/dev/null)`). For each rejected path, two stale-base shapes mean "push back, do not revert": (a) **shared history at fork** — `[ -n "$MERGE_BASE" ] && git cat-file -e "$MERGE_BASE:<path>"` succeeds, meaning the file existed at the fork point and is not a scope expansion you added; (b) **fork-point-vs-tip drift** — the file is absent at `$MERGE_BASE` but `git cat-file -e "origin/$BASE:<path>"` succeeds and `git cat-file -e HEAD:<path>` fails, meaning `main` added the file *after* the fork and it was never on the branch. In either case, push back with the per-commit diff (`git log "$BASE..HEAD" --stat`, then `git diff <commit>^..<commit> --stat` for the suspect commit) instead of reverting shared history; skip the salvage flow below. The merge-base form supersedes the legacy `BASE:<path>` + `HEAD:<path>` conjunction, which resolved `$BASE` to the current `origin/$BASE` tip and missed arm (b).
