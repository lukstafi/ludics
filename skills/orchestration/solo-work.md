...
{{#IF PROPOSAL_PATH}}
Before signaling done, re-read `{{PROPOSAL_PATH}}` and walk through each acceptance criterion, stating explicitly (in your thinking) how the implementation satisfies it. Each criterion's mental walk should also name the harness condition that makes the test exercise that AC — a test that passes whether or not the condition holds does not enforce it. See [harness instantiation](../../docs/orchestration-patterns.md#harness-instantiation) and [mutation evidence](../../docs/orchestration-patterns.md#mutation-evidence). Write the status file only once every criterion is met.
{{/IF}}
