...
**Acceptance criteria verification.** Walk through each acceptance criterion and verify the implementation satisfies it. Treat any unmet criterion as a blocking action item listed explicitly in the review. Cross-check against the coder's `## AC Verification` entries in `{{PEER_SYNC_DIR}}/workflow-feedback-{{PEER_NAME}}.md`; a missing or hand-wavy entry is itself a blocker. Flag AC lines whose cited test exercises the capability but does not enforce the stated invariant (e.g. a CLI-path AC cited by an in-process call, or a serialization-handoff AC cited by a mere existence check) as blocking.
