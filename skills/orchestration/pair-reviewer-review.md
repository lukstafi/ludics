...
Before flagging apparent deletions as scope violations, cross-check with `git log main..HEAD --stat` (per-commit summary) and `git diff <commit>^..<commit> --stat`. Deletions that appear in `main..HEAD` but not in any per-commit diff are main-side drift from a stale branch; the remedy is rebase, not scope pushback. If you have already drafted `REQUEST_CHANGES` naming a file as a deletion or out-of-scope addition, resolve the base branch (`BASE=$(git symbolic-ref --quiet --short refs/remotes/origin/HEAD 2>/dev/null | sed 's@^origin/@@' || echo main)` — substitute `master` or your repo's actual default if `origin/HEAD` is unset) and the fork point (`MERGE_BASE=$(git merge-base "origin/$BASE" HEAD 2>/dev/null)`). If `[ -z "$MERGE_BASE" ]` (orphan branch, unfetched remote, shallow clone), neither arm below is conclusive — leave the verdict open rather than firing on unrelated state. Otherwise, check both stale-base shapes: (a) **shared history at fork** — `[ -n "$MERGE_BASE" ] && git cat-file -e "$MERGE_BASE:<path>"` succeeds, meaning the file was already at the fork point; (b) **fork-point-vs-tip drift** — `[ -n "$MERGE_BASE" ] && ! git cat-file -e "$MERGE_BASE:<path>"` (file absent at fork point) AND `git cat-file -e "origin/$BASE:<path>"` succeeds AND `git cat-file -e HEAD:<path>` fails, meaning `main` added the file after the fork and the branch never carried it. Either shape is the smoking gun for stale-base drift, not for scope violation. The merge-base form supersedes the legacy `BASE:<path>` + `HEAD:<path>` conjunction, which read the current `origin/$BASE` tip instead of the fork point and missed arm (b).
