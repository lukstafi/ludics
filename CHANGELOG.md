...
- **Once-daily outbound staging→upstream push** ([gh-ludics-540](https://github.com/lukstafi/ludics/issues/540)). Sibling of the inbound fast-forward: keepalive tick attempts `git push upstream origin/<default>:<default>` on each project with `outbound_sync_enabled: true` (new optional `ProjectConfig` field, default `false`). Pull-before-push (`git fetch origin <default>` then local `merge --ff-only`) prevents pushing a stale local view that would drop worker-pushed commits. Fast-forward-only by construction: client-side `git merge-base --is-ancestor upstream/<u> origin/<o>` pre-check plus git's server-side rejection of non-ff refspec pushes (no `--ff-only` flag — unsupported by `git push` in git 2.50.1; no `--force` / `--force-with-lease`). Throttled per-project via `mag/last-outbound-fast-forward-<project>.epoch`. Auth failures surface only via health-check's new `outbound-staging-ff-stale:<project>` stable key (warning 48h / critical 72h); no `ludics notify outgoing` is emitted. Briefing-lag adds an "outbound sentinel stale > 48h" annotation alongside the existing `fetch data is ~Nh old` note. Also extends `RunGitResult` with optional `stderr` so the credentials classifier can read `git push` stderr literals. New entry point `syncUpstreamMainFromStaging` in `src/staging-ff.ts`; new wrapper `runStagingOutboundPushTick` in `src/mag.ts` next to `runStagingFastForwardTick`. Emits `staging_outbound_fast_forwarded`, `staging_outbound_fast_forward_diverged`, `staging_outbound_local_behind`, `staging_outbound_credentials_missing`, and `staging_outbound_error` events.
