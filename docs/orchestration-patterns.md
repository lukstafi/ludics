...
5. *A mechanical lint (`lint:state-migration`) enforces co-change on the explicitly-allowlisted persisted shapes (`PERSISTED_TYPES` in `scripts/lint-state-migration.ts`).* Reviewer-side guidance (the data-shape paragraph in `skills/orchestration/pair-reviewer-plan-review.md`) remains the catch-all for in-place shape changes that escape the field-set extractor (e.g., scalar→record narrowing without a key rename) and for persisted shapes outside the allowlist.

**Mechanical enforcement.** `lint:state-migration` (`scripts/lint-state-migration.ts`) compares the field set of each allowlisted type (`OrchestrationState`, `OrchestrationConfig`, `SlotData`, `TmuxSlotState`, `SessionSweepState`, `CleanupEntry`, `PreemptStash`) against a checked-in `scripts/snapshots/state.shape.snapshot.json`. Any field touch (add, remove, rename) requires a paired `git diff` body hit inside the named migrator (per `MIGRATOR_SITES` in the same script — `migrateState`, `readTmuxSlotState`, `loadDeferredCleanups`, `readStash`, `loadSessionSweepState`, `normalizeTaskId`) AND a touched test file matching `*.migrate*.test.ts` / `state*.test.ts` / a sibling test in the same directory. Run `bun scripts/lint-state-migration.ts --update` to refresh the snapshot after the migrator and test land. The lint does **not** detect in-place type narrowing without a key rename (e.g. `foo: string` → `foo: "a" | "b"`); reviewer guidance remains the catch-all for those cases.
...
**Procedure (diff commands).** When assessing scope on a feature branch, prefer the per-commit view over the cumulative view: the branch may be stale against its base. `git diff main..HEAD --stat` is cumulative and conflates main-side drift with this branch's own work — on a branch that forked hours ago while other PRs landed, it can attribute unrelated deletions and additions to this branch. `git log main..HEAD --stat` (per-commit summary across the branch) and `git diff <commit>^..<commit> --stat` (a single commit's actual change against its own parent) attribute changes to the branch's own commits. Rule: when `main..HEAD` shows large deletions but the per-commit diffs do not, that is main-side drift from a stale branch, not a scope violation — the remedy is `git rebase origin/main` and re-review, not scope pushback or manual restoration. The same logic applies to any base branch, not only `main`. Runner-side stale-base detection (`warnStaleBase`, fires on entry to `plan`, `work`, `plan-review`, and `review`) is the prevention layer; if a reviewer is staring at a suspicious cumulative diff, the warning either did not fire or was missed. When an individual file claim is in dispute (reviewer says "X is out of scope," coder pushes back), the canonical per-file verification resolves the actual fork point and checks two stale-base shapes against it. Resolve the base branch with `BASE=$(git symbolic-ref --quiet --short refs/remotes/origin/HEAD 2>/dev/null | sed 's@^origin/@@' || echo main)` and the fork point with `MERGE_BASE=$(git merge-base "origin/$BASE" HEAD 2>/dev/null)`. If `[ -z "$MERGE_BASE" ]` (orphan branch, unfetched remote, shallow clone), neither arm is conclusive — fall through to ordinary salvage rather than firing, since "no shared ancestor" cannot distinguish drift from genuine addition. Otherwise: arm (a) **shared history at fork** — `[ -n "$MERGE_BASE" ] && git cat-file -e "$MERGE_BASE:<path>"` succeeds, meaning the file was present at the fork point and the branch did not add it; arm (b) **fork-point-vs-tip drift** — `[ -n "$MERGE_BASE" ] && ! git cat-file -e "$MERGE_BASE:<path>"` (file absent at fork point) AND `git cat-file -e "origin/$BASE:<path>"` succeeds AND `git cat-file -e HEAD:<path>` fails, meaning `main` added the file after the fork and it was never on the branch. Either arm short-circuits the salvage/revert flow with the per-commit-diff push-back. The merge-base form supersedes the legacy `BASE:<path>` + `HEAD:<path>` conjunction, which read the current `origin/$BASE` tip rather than the fork point and missed arm (b). Avoid hard-coding `main` — the same logic applies to `master`, `trunk`, or any custom default branch.
...
Every new CLI subcommand that does `join(tasksDir(), \`${id}.md\`)` (or any equivalent path-from-untrusted-input join) needs the `TASK_ID_RE` guard at the start of its case body, *before the join* — a "task not found" check fires *after* the join and so misses path-traversal IDs like `../decoy/sibling`, where the joined path resolves to a writable file outside the protected directory and the existence check trivially passes (or, worse, the file exists and gets clobbered). When mirroring a sibling CLI surface, audit *all* of its guards: walking the sibling's body line by line is a checklist task, and the diff between the sibling and the new case body should be approximately *only* the differences that name the new behaviour — input enum, error messages, value field. Anything else is a guard you forgot to copy. Concrete trigger: PR #476 round 2 caught a path-traversal vulnerability in a freshly-shipped `tasks status` subcommand whose case body mirrored `tasksSetPriority` (`src/tasks/index.ts:657`) but missed its `TASK_ID_RE.test(taskId)` line; the fix added the guard plus a real-decoy regression test. Distinct from the neighbouring `### CI drift files` rule (lint-enforced drift between *paired* files); this rule is a single-file, sibling-mirroring discipline that no current lint enforces — pair it with [Real-decoy + byte-identity for path-safety probes](ac-rigor-reference.md#real-decoy--byte-identity-for-path-safety-probes) (`ac-rigor-reference.md` § Vacuous-harness family) so the production-guard rule and its regression-test recipe land together.
