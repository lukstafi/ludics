...
- `resolveQueueRequestCommand()` at `src/mag.ts:1247` — three-tier dispatcher; the `message` branch returns `null` from the slash-command path today, so every command currently reaching `triggerSkill` via this seam is automated. The structural rule is forward-compatible if free-form `content` messages start being fed via this path later.
...
- **Future free-form messages:** If a `{ action: "message", content: "…" }` entry ever reaches this feeder (currently impossible — Tier-3 returns `null`), the structural rule already does the right thing: no prefix. No code change needed when that path gets wired up.
