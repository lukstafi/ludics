...
- [ ] No grep hit for `detectDefaultBranchesAuthoritative` remains anywhere in `src/`, `docs/`, or `dashboard/` after the change. (Historical retrospective files in `retrospectives/` may still mention it — those are immutable records and stay as-is.)
